...
function cartCommandSettingsAddQuantity(addSettings, addItem) {
  ...
  input.addEventListener("input", () => cartCommandUpdatePriceQuantity(addItem.id, input.value, addItem))
...
function cartCommandShowQuantity() {
  ...
  const totalOfAllProducts = cart.reduce((totalOfAllProducts, item) => totalOfAllProducts + item.quantity, 0)
